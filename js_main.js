...
const loadHousekeepingButton = document.getElementById('load-housekeeping-button');
...
loadHousekeepingButton.addEventListener('click', loadHousekeepingPresets);
...
function loadHousekeepingPresets() {
    const presets = ["Make Bed", "Clean Bathroom", "Vacuum Floor", "Dust Surfaces", "Empty Trash", "Restock Supplies", "Mop Floor"];
    presets.forEach(preset => {
        if (!elements.includes(preset)) {
            elements.push(preset);
        }
    });
    renderElements();
    updateElementProgress();
    showTransitionNotification("Housekeeping presets loaded");
}
...
function logProcess() {
    ...
    // Core Work Study Metrics
    const rating = isDelayActive ? 100 : parseInt(ratingInput.value) || 100;
    const allowance = isDelayActive ? 0 : parseInt(allowanceInput.value) || 0;
    
    const normalTimeMs = (elapsedTimeInMs * rating) / 100;
    const standardTimeMs = normalTimeMs * (1 + (allowance / 100));
    ...
    entry.allowance = isDelayActive ? "N/A" : `${allowance}%`;
    entry.standardTime = isDelayActive ? entry.observedTime : timeToString(standardTimeMs);
    entry.notes = notesInput.value;

    // For debugging
    console.log("Elapsed time:", elapsedTimeInMs, "ms");
    console.log("Observed time:", entry.observedTime);
    console.log("Standard time:", entry.standardTime);
...
function insertRowIntoTable(domReference, entry) {
    ...
    newRow.insertCell(7).textContent = entry.allowance;
    newRow.insertCell(8).textContent = entry.standardTime;
    newRow.insertCell(9).textContent = entry.notes;
...
function exportToCSV() {
    ...
    const headers = ["Element", "Category", "Start Time", "Stop Time", "Observed Time", "Rating (%)", "Normal Time", "Allowance (%)", "Standard Time", "Notes"];
